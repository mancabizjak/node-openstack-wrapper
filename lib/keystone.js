...
/* Checks (validates) provided authentication token */
Keystone.prototype.checkToken = function(adminToken, subjectToken, cb)
{
    var self = this;
    var request_options = this.getRequestOptions(adminToken, '/auth/tokens', true);
    request_options.headers['X-Subject-Token'] = subjectToken;
    request_options.logPath = 'api-calls.keystone.tokens-check';

    this.request.get(request_options, function(error, response, body){
        validationResult = undefined;
        if(osutils.isRequestError(error, response))
        {
          if (response.statusCode == 404) { /* Token does not exist/expires --> is not valid */
        ...
        if (response.statusCode == 200) { /* Expected status code */
          validationResult = true;
        }
        body.token_valdation_result = validationResult;
        cb(null, self.mangleObject('TokenValidationResult', body));  
      });
}

/* Gets token revocation list */
Keystone.prototype.getRevokedTokensList = function(adminToken, cb)
{
  var request_options = this.getRequestOptions(adminToken, '/auth/tokens/OS-PKI/revoked', true);
...
/* Gets Keystone's certificates */
Keystone.prototype.getCertificates = function(cb)
{
  var self = this;
  this.getCACert(function(error, caCert) {
    if (!error) {
      self.getSigningCerts(function(error, signingCerts) {
        if (!error) {
          cb(null, caCert, signingCerts);
          return;
        }
      });
    }
...
/* Gets Keystone's CA certificate */
Keystone.prototype.getCACert = function(cb)
{
    var request_options = this.getRequestOptions(null, '/OS-SIMPLE-CERT/ca', true);
...
/* Gets Keystone's signing certificate(s) */
Keystone.prototype.getSigningCerts = function(cb)
{
    var request_options = this.getRequestOptions(null, '/OS-SIMPLE-CERT/certificates', true);
    request_options.headers = {};
    this.request.get(request_options, function(error, response, body) {
      if(osutils.isRequestError(error, response)) {
        cb(osutils.getRequestError(error, response, body));
...
Keystone.prototype.getDomainScopedToken = function(username, password, domain, cb)
{
  var self = this;
  var auth_data = {
    auth:{
